...
try:
    modelTie = load_model('groundZero/tie.h5')
    modelWL = load_model('groundZero/WL.h5')
    print('Models loaded! Training now...')
except:
    print("Can't find models! Creating new ones...")
    modelTie = Sequential()
    modelTie.add(Dense(81, activation='relu', input_dim=9))
    modelTie.add(Dense(9, activation='softmax'))
    modelTie.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])

    modelWL = Sequential()
    modelWL.add(Dense(81, activation='relu', input_dim=9))
    modelWL.add(Dense(9, activation='softmax'))
    modelWL.compile(loss='categorical_crossentropy', optimizer='adam', metrics=['accuracy'])
...
def nextMoveTie(boards):
    board = boards[-1]
    predict = modelTie.predict(np.array([board]))
...
def nextMoveWL(boards):
    board = boards[-1]
    predict = modelWL.predict(np.array([board]))
    nextMove = np.argmax(predict)
    if random.choice(range(10)) == 0:
        nextMove = randMove(board)
    while board[nextMove] != 0:
...
def calRewardsWL(boards):
    ...
    if winTie == 1:
        ...
        reward = -5
    boards.pop()
    for i,j in enumerate(movesOrd):
        currPlayer = 0
        rMatrix = [0,0,0,0,0,0,0,0,0]
        rMatrix[j] = reward*rDecay**(moves-i-1)
...
for x in range(1000):
    ...
    if x%20==0:
        for y in boards:
            ...
            print_board(y)
    _=boards.pop()
    boards,WL,tie = np.array(boards),np.array(WL),np.array(tie)
    modelTie.fit(boards,tie,validation_split=0.25, epochs=10, batch_size=16, verbose=0)
    modelWL.fit(boards,WL,validation_split=0.25, epochs=10, batch_size=16, verbose=0)
    if x%50==0:
        modelTie.save('groundZero/tie.h5')
        modelWL.save('groundZero/WL.h5')